<?php
    include 'connect.php';
      ...
      ?>
        ...
        <?php
        $stmt = $conn->prepare("SELECT * FROM ads  WHERE service = ? AND  servicetype = ? AND userid != $myid AND visibility = 1 ORDER BY id DESC LIMIT 1 ");
        ...
        $stmt2 = $conn->prepare("SELECT * FROM  ads WHERE  service = ? AND servicetype = ? AND status = 1 AND visibility = 1 ORDER BY   RAND() LIMIT 0,$total;");
        $stmt2->execute(array($service,$servicetype));
        $ad = $stmt2->fetch();
