    ...
    if ( $_SESSION['type'] == 2)
{
    $page = isset($_GET['page']) ? $_GET['page'] : 'manage';
        ...
        if ($page == 'manage')
        {
          ...
          $stmt = $conn->prepare("SELECT * FROM cp ORDER BY id $ord");
                    $stmt->execute();
                    $posts = $stmt->fetchAll();
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="left-header management-header">
                      <h1>total free points company </h1>
                      <p class="tt">total cp<?php echo Total($conn, 'cp') ?> .</p>
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="right-header management-header">
                      <div class="btns">
                        <a href="cp.php?page=add" class="add-btn">add new free points company </a>
                      </div>
                    </div>
                  </div>
                      ...
                      <ul>
                        <li class=" dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                  <a class="dropdown-item" href="cp.php?page=manage&ordering=ASC">

                                    ASC
                                  </a>

                                  <a class="dropdown-item" href="cp.php?page=manage&ordering=DESC">
                                    DESC
                                  </a>
                                </div>
                              </li>
                          ...
                          <tbody>
                            ...
                            foreach($posts as $post)
                            {
                              ...
                              <tr>
                                ...
                                <td>
                                  <a href="<?php echo $post['image'] ?>" target="_blank">link <i class="fas fa-eye"></i> </a>
                                </td>
                                        ...
                                        <ul>
                                          <li class=" dropdown">
                                                  ...
                                                  <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                                    ...
                                                    <a class="dropdown-item" href="cp.php?page=delete&id=<?php echo $post['id'] ?>">
      ...
      <div class="add-default-page add-post-page  add-product-page " id="add-page">
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-md-8">
              <form class="add-fomr" method="POST" action="cp.php?page=insert" enctype="multipart/form-data"  id="ca-form-info"  >
                <h3>add new free points company <i class="fas fa-plus"></i>  </h3>
                  <div class="row">
                    <div class="form-group col-12">
                      <label for="title">name</label>
                      <input type="text" name="name" id="title" placeholder=" " class="form-control">
                    </div>
                    <div class="form-group col-12">
                      <label for="click">url / link</label>
                      <input type="text" name="logo" id="click" placeholder=" " class="form-control">
                    </div>


                    <div class="form-group col-12">
                      <label for="click">points</label>
                      <input type="text" name="points" id="click" placeholder=" " class="form-control">
                    </div>
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST')
      {
        ...
        if (empty($name))
        {
          $formErrors[] = 'name is requried';
        }

        if (empty($logo))
        {
          $formErrors[] = 'link  is required';
        }
        if (empty($points))
        {
                          ...
                          if (empty($formErrors))
                            {
                                ...
                                $stmt = $conn->prepare("INSERT INTO cp(image,name,points,created)
                                VALUES(:zn,:zimg, :zdesc, now() )");
                                $stmt->execute(array(
                                  'zn' => $logo,
                                  'zimg' => $name,
                                  'zdesc' => $points
                              ...
                              The cp  has been successfully added
                                </div>
                                <?php
                                header('location: cp.php?page=manage');
                              }
    ...
    elseif ($page == 'edit') {
      $pageTitle = "cpedit page";
      include 'init.php';

      $id = isset($_GET['id']) && is_numeric($_GET['id']) ? intval($_GET['id']) : header('location: cp.php');
      $stmt = $conn->prepare("SELECT * FROM cpWHERE id= ? LIMIT 1");
              ...
              <div class="row justify-content-center">
                ...
                <div class="col-md-12">
                  <div class="page-direction-bnts">
                    <ul>
                      ...
                      <li>
                        <a href="cp.php?page=edit&id=<?php echo $postinfo['id'] ?>">
                ...
                <div class="col-md-7">
                  <div class="use-fl-info">
                    <form class="add-fomr" method="POST" action="cp.php?page=update" enctype="multipart/form-data"  id="ca-form-info"  >
                        <div class="row">
                          <div class="form-group col-12">
    ...
    elseif ($page == 'update') {
      ...
      $id=$_POST['id'];
                            ...
                            if ($checkpst > 0 )
                            {
                                ...
                                ?>
                                  <div class="container">
                                    <a href="cp.php?page=edit&id=<?php echo $id ?>">click here to back to latest page</a>
                                  </div>
                                <?php
                                ...
                                if (empty($formErrors))
                                {
                                  ...
                                  $stmt = $conn->prepare("UPDATE cpSET name = ? ,image=?,description = ? WHERE id=? LIMIT 1  ");
     ...
     include 'init.php';
      ...
      {
        $id = isset($_GET['id']) && is_numeric($_GET['id']) ? intval($_GET['id']) : header('location: cp.php');;
        $stmt = $conn->prepare("SELECT * FROM cp WHERE  id = ? LIMIT 1");
        $stmt->execute(array($id));
        $check = $stmt->rowCount();

        if ($check > 0 )
        {
          $stmt = $conn->prepare("DELETE FROM cp WHERE  id = :zid");
